...
ggmatrix(list(p1,p1.1), 2,1,yAxisLabels = c("LMM vs. OLS", "BNP vs. OLS"))
...
qqq1<- ggmatrix(list(q11,q21,q31,
                     q12,q22,q32,
                     q13,q23,q33),3,3,xAxisLabels = c("OLS","LMM","BNP"),
...
mv.trend.df <- data.frame(rbind(cbind(ols, sigma = sqrt(ind_est$sigma2-.1), type="OLS"),
                                cbind(bnp, type="BNP"))
)
mv11 <- filter(mv.trend.df, type=="OLS") %>%
...
mv21 <- filter(mv.trend.df, type=="OLS") %>%
...
mv31 <- filter(mv.trend.df, type=="OLS") %>%
...
qqq2<- ggmatrix(list(mv11,mv12,
                     mv21,mv22,
                     mv31,mv32),3,2,xAxisLabels = c("OLS","BNP"),
                yAxisLabels = c(paste(expression(paste(hat(sigma),"  vs.  ",hat(beta)[1]))),
                                paste(expression(paste(hat(sigma),"  vs.  ",hat(beta)[2]))),
                                paste(expression(paste(hat(sigma),"  vs.  ",hat(beta)[3])))),
...
p1 <- diffHexHist(bnp12,ols12, 30, .0008, "")+ggtitle("BNP-OLS")+no_panel+no_axes
p2 <- diffHexHist(lmm12,ols12, 30,.0008, "")+ggtitle("LMM-OLS")+theme(legend.position="none")+no_panel+no_axes
p3<- diffHexHist(bnp12,lmm12, 30,.0008, "")+ggtitle("BNP-LMM")+theme(legend.position = "none")+no_panel+no_axes
...
q1 <- diffHexHist(bnp13,ols13, 20, .0008, "")+ggtitle("BNP-OLS")+no_panel+no_axes
q2 <- diffHexHist(lmm13,ols13, 20,.0008, "")+ggtitle("LMM-OLS")+theme(legend.position="none")+no_panel+no_axes
q3<- diffHexHist(bnp13,lmm13, 20,.0008, "")+ggtitle("BNP-LMM")+theme(legend.position = "none")+no_panel+no_axes
...
r1 <- diffHexHist(bnp23,ols23, 30, .0008, "")+ggtitle("BNP-OLS")+no_panel+no_axes
r2 <- diffHexHist(lmm23,ols23, 30,.0008, "")+ggtitle("LMM-OLS")+theme(legend.position="none")+no_panel+no_axes
r3<- diffHexHist(bnp23,lmm23, 30,.0008, "")+ggtitle("BNP-LMM")+theme(legend.position = "none")+no_panel+no_axes
...
ggmatrix(list(p1,p2,p3,q1,q2,q3,r1,r2,r3,s1, blank_panel, blank_panel),nrow = 4, ncol=3,yAxisLabels = c(paste(expression(paste(beta[2]," vs. ",beta[1]))),
         ...
         xAxisLabels=c(paste(expression(paste("BNP", " vs. ", "OLS"))),
                       paste(expression(paste("LMM", " vs. ", "OLS"))),
                       paste(expression(paste("BNP", " vs. ", "LMM")))),
