...
the time required goes from to process $n$ elements is $O(n)$ to $O(n/P + \log_2P)$ vs $O(n)$ for the serial algorithm. This is the same order as for the parallel reduction. Parallel scans are implemented in Thrust, called with \code{inclusive\_scan} for a full scan and \code{exclusive\_scan} for a prescan.
...
Here $M_k$ is the number of $g$ for which $\zeta_g = k$. The main hurdle here is the computation of $\hat{\lambda}$. This is done in three steps, each parallel over $k$: dot products for $\beta_k^\top \sum_{g:\zeta_g=k}X^\top W_g y_g$, quadratic forms for $\beta_k^\top\left( \sum_{g:\zeta_g=k} X^\top W_g X \right) \beta_k$ and a \code{for\_each} call to combine the terms.
