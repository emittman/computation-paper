...
Since the full conditional only depends on the value of $K$, $a_\alpha$ and $b_\alpha$, which are constant and the scalar quantity $\pi_K$, this step can be conveniently performed by the CPU. 
...
\begin{itemize}
\item \code{thrust::for\_each}: This algorithm accepts a void function and an iterator. The serial version increments the iterator, passing each element to the function in turn. The parallel implementation produces the same results, only in thread parallel fashion. The \code{thrust::zip\_iterator} is very useful here, as it iterates over a \code{thrust::tuple} allowing for an AoS approach.

\item \code{thrust::reduce/reduce\_by\_key} As described in section \ref{sec:computation}, Reduce and Scan are defined for any associative binary operators. Reduce by key accepts a key argument and a compatible binary predicate that identifies changes in the key. For example, for the binary predicate \code{x == y}, the key \code{\{1,1,1,2,2,1\}} would have the result be three quantities, the reductions of the first three values, the next two and the last, respectively.

\item \code{thrust::inclusive/exclusive\_scan/scan\_by\_key} 

\end{itemize}

 through a call to the use of a custom functor with a call to \code{for\_each}, operating on a \code{zip\_iterator} formed from a \code{tuple} of \code{permutation\_iterator}s which permit random access to the leading positions of the continguous blocks of memory containing $\tilde{\beta}_k$ and $X^\top W_g y_g$.

CUBLAS \cite{cublas} has implemented BLAS/LAPACK linear algebra
...
, which can be resolved using the dgemm routine via the CUBLAS API. 
\subsection{Multimodality and initialization}
\label{subsec:initialization}
An issue with mixture models is multimodality due to non-identifiability of the mixture components. \citet{papas} found that, for the DPMM, although the cluster weights are stochastically ordered, they can still be highly multi-modal. As a remedy, they proposed label-switching moves to improve mixing across indices. Since our interest is in the gene specific parameters, $\beta_g,\sigma^2_g$, rather than the clusters, $\tilde{\beta}_g,\tilde{\sigma}^2_g$, we are not concerned with the multimodality. However, we find empirically that poor initialization will lead to poor mixing, due to an issue related to cluster indexing. In particular, especially when $p$ is moderately large, during the initial stages the $K^{th}$ cluster can become occupied. This leads to large draws for $\alpha$, with the implication that there is no preference in the model for allocation to lower clusters. To remedy this problem, we reorder the cluster indices by their current value of $\pi_k$ after a short initial run, then proceed to subsequent burn-in iterations. Figure \ref{fig:max-id} shows traceplots of a series of Markov chains where reordering has been applied to each. The top frame shows the maximum occupied cluster ID and the bottom frame shows the value of $\alpha$. This example is simulated data with $G=40,000,\, p=6,\, \mbox{and } K=8000$.
...
\item gene-specific posterior probabilities of conjunctions of linear inequalities of the elements of $\beta_g$, i.e., $\op{Pr}\left(c_1\beta_g > 0 \land \ldots \land c_t\beta_g\right)$.
\end{enumerate}
